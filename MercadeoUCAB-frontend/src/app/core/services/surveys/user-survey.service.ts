...
export class UserSurveyService {
  ...
  getAvailableSurveys(person_id): Observable<Respuesta>{
    return this.http.get<Respuesta>(serverURL + 'survey/available/' + person_id)
      .pipe(catchError(this.processHTTPMessageService.handleError))
  }
  ...
  postAnswers(study_id, person_id, survey): Observable<Respuesta> {
    ...
    return this.http.post<Respuesta>(serverURL + 'survey/take/' + study_id + '/' + person_id, survey, httpOptions)
      .pipe(catchError(this.processHTTPMessageService.handleError))
  }
}
